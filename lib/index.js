                ...
                if (el.closed || el.fill) {
                    ...
                    el.draws.forEach((item, k) => {
                        ...
                        if (el.draws[k + 1] && el.draws[k + 1].type == 'lineTo') {
                            ...
                            node.pointsSet = [item.points, el.draws[k + 1].points];
...
traverse(ast, {
    enter(p) {
        ...
        if (t.isExpressionStatement(p.node) && t.isCallExpression(p.node.expression) && t.isMemberExpression(p.node.expression.callee) && t.isIdentifier(p.node.expression.callee.property)) {

            if (p.node.expression.callee.property.name == 'beginPath') {
                phaseCheck();
            }

            if (p.node.expression.callee.property.name == 'moveTo' || p.node.expression.callee.property.name == 'rect' || p.node.expression.callee.property.name == 'strokeRect' || p.node.expression.callee.property.name == 'fillRect' || p.node.expression.callee.property.name == 'arc') {
            ...
            if (p.node.expression.callee.property.name == 'moveTo') {
                el.draws.push({
                    type: 'moveTo',
                    points: [extractArgument(p.node.expression.arguments[0]), extractArgument(p.node.expression.arguments[1])]
                });
            }
        ...
        if (t.isExpressionStatement(p.node) && t.isAssignmentExpression(p.node.expression) && t.isMemberExpression(p.node.expression.left) && t.isIdentifier(p.node.expression.left.property)) {
